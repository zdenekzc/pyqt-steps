...
void Compiler::info (string msg)
{
   cout << msg << endl;
}

void Compiler::error (string msg)
{
   cerr << msg << endl;
...
void Compiler::compileFile (string fileName)
{
    // signal(SIGSEGV, handler);
    int argc = 2;
    const char * argv [2] = { "execute", fileName.c_str () };
    execute (argc, argv);
}

void Compiler::compile (vector <string> options, vector <string> libraries)
{
    // signal(SIGSEGV, handler);
